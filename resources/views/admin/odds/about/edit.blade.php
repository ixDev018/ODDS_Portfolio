...
<a href="{{ route('odds.admin.about.index') }}" class="pe-back">
    <i class="fa-solid fa-arrow-left text-[10px]"></i>
    <span>Back to About Sections</span>
</a>

<form id="about-form" action="{{ route('odds.admin.about.update', $section->id) }}" method="POST" enctype="multipart/form-data" class="pe-shell">
    @csrf
    ...
    <div class="pe-editor-col">
        <div class="pe-editor-header">
            <input type="text" name="title" required value="{{ old('title', $section->title) }}" placeholder="Untitled Section / Article..."
                   class="pe-title-input" autofocus>
            <textarea name="subtitle" rows="2" placeholder="Write a short summary or introductory hook for this section..."
                      class="pe-desc-input">{{ old('subtitle', $section->subtitle) }}</textarea>
        </div>
    ...
    <div class="pe-sidebar">
        <div class="pe-sidebar-header">
            <span class="pe-sidebar-header-label">Section Settings</span>
            <span class="font-mono text-[9px] {{ $section->is_active ? 'text-emerald-400' : 'text-[#875af5]' }} font-bold">
                {{ $section->is_active ? 'PUBLISHED' : 'DRAFT' }}
            </span>
        </div>

        <div class="pe-sidebar-scroll">
            <div>
                <label class="odds-label">Category / Badge</label>
                <input type="text" name="category" value="{{ old('category', $section->category) }}" placeholder="e.g. DNA, Vision, Team, Origin" class="odds-input">
            </div>

            <div class="grid grid-cols-2 gap-2">
                <div>
                    <label class="odds-label">Author</label>
                    <input type="text" name="author" value="{{ old('author', $section->author) }}" placeholder="ODDS Core" class="odds-input">
                </div>
                <div>
                    <label class="odds-label">Read Time</label>
                    <input type="text" name="read_time" value="{{ old('read_time', $section->read_time) }}" placeholder="3 min read" class="odds-input font-mono text-xs">
                </div>
            </div>
            ...
            <div class="space-y-1.5">
                <div class="flex items-center justify-between">
                    <label class="odds-label mb-0">Cover Image</label>
                ...
                <div id="cover-dropzone" class="relative group aspect-[16/9] w-full rounded-xl border-2 border-dashed border-[#2b2b36] hover:border-[#875af5] bg-[#0d0d12] flex flex-col items-center justify-center p-3 text-center cursor-pointer transition-all overflow-hidden">
                    <div id="cover-placeholder" class="flex flex-col items-center justify-center space-y-2 pointer-events-none {{ !empty($section->cover_image) ? 'hidden' : '' }}">
                    ...
                    <div id="cover-preview-container" class="{{ empty($section->cover_image) ? 'hidden' : '' }} absolute inset-0 w-full h-full">
                        <img id="cover-preview-img" src="{{ $section->cover_image ?? '' }}" alt="Cover Preview" class="w-full h-full object-cover">
            ...
            <div class="pt-2 border-t border-[#22222a] space-y-2">
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ $section->is_active ? 'checked' : '' }} class="rounded bg-[#0b0b0e] border-[#22222a] text-[#875af5] focus:ring-0">
                    <span class="text-xs text-gray-300 font-semibold">Active & Published</span>
                </label>
            </div>
        </div>

        <div class="pe-sidebar-footer flex items-center gap-2">
            <button type="submit" class="odds-btn-primary flex-1">
                <i class="fa-solid fa-check text-xs"></i>
                <span>Update Section</span>
            </button>
            <a href="{{ route('portfolio.about') }}#section-{{ $section->id }}" target="_blank" class="odds-btn-secondary px-3" title="Preview on live page">
...
<div id="slash-menu" class="pe-slash-menu hidden">
    ...
    <div class="pe-slash-item" data-type="image">
        ...
        <div>
            ...
            <div class="text-[10px] text-gray-400">Upload inline story image</div>
        </div>
    </div>
</div>
@endsection
...
@php
    $existingBlocks = $section->body_content ?? [];
    ...
    if (empty($existingBlocks)) {
        $existingBlocks = [
            ['type' => 'heading2', 'content' => $section->title ?? 'Section Heading'],
            ['type' => 'paragraph', 'content' => $section->subtitle ?? 'Write your story narrative here...']
        ];
    }
@endphp

const initialBlocks = {!! json_encode($existingBlocks) !!};

document.addEventListener('DOMContentLoaded', () => {
    if (Array.isArray(initialBlocks) && initialBlocks.length > 0) {
        initialBlocks.forEach(b => createBlockElement(b));
    } else {
        createBlockElement({ type: 'paragraph', content: '' });
    }
    initSortable();
});
...
function getPlaceholder(type) {
    switch(type) {
        ...
        case 'callout': return 'Highlight key takeaway or team note...';
        case 'code': return '// write or paste code here...';
        case 'bullet': return 'List item...';
        case 'numbered': return 'Numbered step...';
        default: return "Type '/' for commands or start writing story...";
    }
}
